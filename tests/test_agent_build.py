...
privilege boundaries intact: a fixed shape -- the three analysts in parallel,
then the Remediator, then synthesis -- analysts read-only and narrowed, the
Remediator gated and write-only, one shared timeline and ledger."""
...
def _leaves(agent):
    """Every LlmAgent in the tree, flattened. The pipeline is two levels deep
    now (the analysts sit inside a ParallelAgent), and most of these assertions
    are about the leaf agents, not about which shell holds them."""
    out = {}
...
@pytest.fixture
def leaves(system):
    return _leaves(system.producer)


def test_pipeline_is_a_fixed_three_stage_sequence(system):
    """Analysts together, then the Remediator, then synthesis.

    The three analysts are concurrent because they are independent -- it is the
    difference between a ~90s answer and a ~30s one (see agent/producer.py).
    The two things that must stay *ordered* are asserted here: the Remediator
    reasons about what the analysts found, and synthesis writes last.
    """
...
def test_each_analyst_writes_a_distinct_output_key_and_feeds_the_ledger(leaves):
    keys = {name: a.output_key for name, a in leaves.items()}
    ...
    # Distinct, so three agents writing session state at the same time cannot
    # overwrite each other -- the fan-out's one real hazard.
    analyst_keys = [keys[n] for n in
                    ("schedule_analyst", "farm_analyst", "crunch_guardian")]
    assert len(set(analyst_keys)) == 3
    for name in ("schedule_analyst", "farm_analyst", "crunch_guardian"):
        assert leaves[name].after_agent_callback is not None


def test_analyst_tool_filters_are_read_only_subsets(leaves):
    by_name = leaves
...
def test_remediator_is_write_only_and_gated(leaves):
    rem = leaves["remediator"]
...
def test_synthesis_has_no_tools(leaves):
    syn = leaves["synthesis"]
