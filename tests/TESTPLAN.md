...
| **Edge — usable from a fresh clone** | auto | `LICENSE` exists and is Apache-2.0; `pyproject.toml` declares that license; `README.md` carries a runnable quickstart (`uv sync`, `uv run pytest`). | `test_reproducibility.py::test_the_repo_is_usable_by_someone_who_just_cloned_it` |
