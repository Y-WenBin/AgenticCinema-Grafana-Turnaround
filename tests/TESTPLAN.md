...
| Visitors are told apart behind the proxy | auto | Every request on Cloud Run shares a peer address, so identity comes from the first `X-Forwarded-For` entry. It is spoofable, which is exactly why the *daily* budget does not depend on it. | `test_engine.py::test_visitors_are_told_apart_by_the_forwarded_address`, `test_limits.py::test_the_visitor_id_is_the_client_not_the_proxy` |
| The playground ships and only calls real routes | auto | `web/` was excluded from `.dockerignore` and `.gcloudignore` as an empty placeholder; it is now the page served at `/`, so an exclusion would deploy a landing page that 404s its own banner — visible only in production. The page is a static file, so every same-origin path it fetches is checked against the app's routes. | `test_reproducibility.py::test_the_playground_ships_in_the_image`, `test_engine.py::test_the_page_only_calls_endpoints_that_exist` |
