...
```bash
uv sync --group dev
uv run pytest -q          # the full suite, no network, no credentials
...
| **Edge — auth failure / expired OAuth / 403 mid-session** | auto | Hosted mode with no bearer raises `HostedMcpNotAuthorized` pointing at `agent.mcp_login`; `agent/run.py` catches it and returns a re-authorize message and a non-zero exit — not a traceback. | `test_mcp_grafana.py::test_hosted_without_a_token_fails_with_a_pointer_to_the_login_flow`, `test_reproducibility.py::test_auth_failure_is_handled_gracefully` |
...
| Case | Kind | What it asserts | Test |
|---|---|---|---|
| Deterministic multi-step execution: plan → act (call Grafana) → format | design + auto | The root agent is a three-stage `SequentialAgent` in exactly that order — the three analysts concurrently, then the Remediator, then synthesis; each analyst writes a distinct `output_key`; `synthesis` has no tools and composes from state. A static question therefore always runs plan (fixed), act (analyst MCP calls), format (`synthesis`). | `test_agent_build.py::test_pipeline_is_a_fixed_three_stage_sequence`, `::test_synthesis_has_no_tools`, `test_reproducibility.py::test_synthesis_prompt_only_consumes_specialist_state` |
...
| **One run path, one front end** | design + auto | `agent/engine.py` builds, drives and scores a run exactly once and returns an outcome; `agent/run.py` renders it as console text and an exit code and reaches into nothing. The split was made when there were two front ends — the CLI and an HTTP service that was ~70% a copy of it — and it is what let the service move to its own repository without touching the pipeline. | `test_engine.py` |
...
| Case | Kind | What it asserts | Test |
|---|---|---|---|
| Startup config validation | auto | With `GOOGLE_CLOUD_PROJECT` / Grafana env unset, `agent.run.ask()` returns exit 2 with a message naming the missing keys. | `test_reproducibility.py::test_startup_refuses_incomplete_config` |
| **Edge — usable from a fresh clone** | auto | `LICENSE` exists and is Apache-2.0; `pyproject.toml` declares that license; `README.md` still mentions `uv sync` and `uv run pytest`. Whether the quickstart *works* is the row below — this one only checks the text, which is all it ever did. | `test_reproducibility.py::test_the_repo_is_usable_by_someone_who_just_cloned_it` |
| **Edge — the zero-config command in the README runs** | auto | `python -m seed.populate --dry-run` in a directory with no filled-in `.env` exits 0, prints `dry run: nothing left this machine`, and says out loud that it borrowed a throwaway salt. Run as a subprocess, because that is how a user runs it. | `test_reproducibility.py::test_the_zero_config_command_the_readme_offers_actually_runs` |
| A real seed still refuses an unset salt | auto | Without `--dry-run`, the same unconfigured state exits 2 naming `TURNAROUND_PSEUDONYM_SALT`, with no traceback. Only a run that exports nothing may borrow a salt. | `test_reproducibility.py::test_a_real_seed_still_refuses_to_run_without_a_salt` |
| Every CI action resolves | auto | Each `uses:` in `ci.yml` is pinned, and `astral-sh/*` to an exact release — that action publishes no moving major tag above v7, so a bare major silently resolves to nothing and every job dies before the suite runs. | `test_reproducibility.py::test_every_ci_action_is_pinned_to_something_that_resolves` |
| A failed re-seed reports why, and stops | auto | `seed.refresh` prints the child's **stderr** on failure (not the last line of stdout, which is always the one step that worked), and a first seed that fails exits 1 instead of retrying a standing misconfiguration every 15 minutes forever. | `test_populate.py::test_a_failed_seed_reports_stderr_not_the_last_thing_that_worked`, `::test_a_seed_that_fails_on_the_first_try_stops_instead_of_looping` |
| Tool spans pair on the call id, not a recycled address | auto | An `execute_tool` span closes even when the before/after `ToolContext` objects are genuinely distinct. Keying on `id(tool_context)` -- for an object the plugin holds no reference to -- worked only because CPython reissues the address it just freed; when an allocation landed in between the span was silently never exported. | `test_observability.py::test_tool_spans_pair_on_the_call_id_not_on_a_recycled_address`, `::test_interleaved_tool_calls_close_their_own_spans` |
...
9. **Serving** — not in this repository; see `docs/SETUP.md` §7.
