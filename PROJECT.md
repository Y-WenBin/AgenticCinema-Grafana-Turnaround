...
(`err-mimir-sample-timestamp-too-old`; measured on `niftysamosa2162`: 65 min
...
points** without touching the network. The same run against `niftysamosa2162`
completes in ~4 s.
...
- **`grafana/evalops/build.py`** → `dashboards/turnaround-evalops.json` (11
  ...
  Verified live on `niftysamosa2162`: full trace, metrics, 6 eval events/run,
